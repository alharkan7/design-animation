...
const FONT_MAP = {
  'libre baskerville': ['Georgia', 0.92], 'cormorant garamond': ['Garamond', 0.88],
  'playfair display': ['Georgia', 0.88], 'source serif 4': ['Palatino Linotype', 0.93],
  'source serif pro': ['Palatino Linotype', 0.93], 'merriweather': ['Georgia', 0.90],
  'lora': ['Palatino Linotype', 0.93], 'eb garamond': ['Garamond', 0.88],
  'gelasio': ['Georgia', 0.93], 'fraunces': ['Georgia', 0.85], 'roboto slab': ['Rockwell', 0.92],
  'inter': ['Segoe UI', 0.93], 'dm sans': ['Segoe UI', 0.93],
  'source sans 3': ['Segoe UI', 0.93], 'source sans pro': ['Segoe UI', 0.93],
  'work sans': ['Candara', 0.93], 'outfit': ['Candara', 0.90],
  'nunito': ['Trebuchet MS', 0.90], 'archivo': ['Franklin Gothic Medium', 0.88],
  'space grotesk': ['Century Gothic', 0.88], 'cabinet grotesk': ['Century Gothic', 0.88],
  'clash display': ['Century Gothic', 0.85], 'satoshi': ['Century Gothic', 0.90],
  'overpass': ['Segoe UI', 0.93], 'barlow': ['Segoe UI', 0.93],
  'instrument sans': ['Segoe UI', 0.93], 'poppins': ['Segoe UI', 0.90],
  'open sans': ['Segoe UI', 0.95], 'lato': ['Segoe UI', 0.95],
  'raleway': ['Trebuchet MS', 0.88], 'montserrat': ['Trebuchet MS', 0.88],
  'roboto': ['Segoe UI', 0.95], 'prompt': ['Trebuchet MS', 0.88],
  'kanit': ['Trebuchet MS', 0.85], 'corben': ['Rockwell', 0.82],
  'jetbrains mono': ['Consolas', 0.92], 'fira code': ['Consolas', 0.92],
  'source code pro': ['Consolas', 0.93], 'inconsolata': ['Consolas', 0.95],
  'courier new': ['Courier New', 1.0],
};
const CATEGORY_FALLBACKS = {
  serif: ['Georgia', 0.92], 'sans-serif': ['Segoe UI', 0.93], monospace: ['Consolas', 0.93],
  cursive: ['Segoe UI', 0.90], fantasy: ['Trebuchet MS', 0.88], 'system-ui': ['Segoe UI', 0.93],
};

function mapFont(name) {
  if (!name) return { name: 'Segoe UI', scale: 0.93 };
  const lower = name.toLowerCase();
  if (FONT_MAP[lower]) return { name: FONT_MAP[lower][0], scale: FONT_MAP[lower][1] };
  ...
  for (const slideData of slidesData) {
    ...
    for (const el of sortedElements) {
      ...
      if (el.type === 'text' && (el.innerHTML || el.plainText)) {
        const mapped = mapFont(el.font?.name);
