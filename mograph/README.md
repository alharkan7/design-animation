...
6. **High-Contrast on Light Backgrounds:** If a sequence is intended for a white/light player background, do not make the UI cards white as they will lose distinction. Use very dark cards with soft drop shadows, and draw pure black traces over light-grey paths.
7. **Synchronized SVG Tracing:** When tracing SVG paths with `stroke-dashoffset`, calculate and use the exact mathematical pixel length of the path. Arbitrary large values will cause the trace to render visibly late and fall out of sync with other elements (like camera panning).
8. **Always Use Easing:** Never use standard `linear` or `ease` for primary motion. Always use custom easing (e.g., `cubic-bezier`) to ensure animations feel premium and intentional.
