...
# Mermaid Diagrams
When the document content would benefit from flowcharts, sequence diagrams, timelines, mind maps, or other diagrams, use Mermaid.js to render them.

## REQUIRED Mermaid Setup
Add BOTH script tags inside \`<head>\`, BEFORE the closing \`</head>\` tag:
\`\`\`html
...
## REQUIRED Mermaid CSS (add to your \`<style>\` block)
Mermaid SVGs render at a small fixed size by default. You MUST include this CSS to make them fill their container:
\`\`\`css
.mermaid-container { display: flex; justify-content: center; align-items: center; flex: 1; width: 100%; min-height: 300px; }
.mermaid-container svg { width: 100% !important; max-width: 100%; height: auto !important; max-height: 70vh; }
\`\`\`

## Mermaid Usage Rules
- Use \`<pre class="mermaid">\` elements for diagram markup (NOT \`<div>\`)
- Place diagrams INSIDE slide \`<section>\` elements like any other content
- Adjust \`theme\` to 'dark' or 'default' to match the slide style
- Adjust \`themeVariables\` colors to match the style preset accent colors
- Keep diagram definitions simple and readable (avoid overly complex graphs)
- **ALWAYS** wrap the \`<pre class="mermaid">\` in a \`<div class="mermaid-container">\` for proper sizing

## Example slide with Mermaid diagram:
\`\`\`html
<section class="slide">
...
flowchart/graph, sequence, class, state, gantt, pie, mindmap, timeline, quadrant, sankey
...
- Include Mermaid.js (v11 CDN) for diagrams when content would benefit from visualization — use <pre class="mermaid"> elements and initialize with mermaid.initialize() on DOMContentLoaded
